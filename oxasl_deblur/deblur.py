...
from oxasl import Workspace, AslImage, image, basil, mask, reg
...
def flatten_mask(mask, thresh_value):
    ...
    if thresh_value > mask.shape[2]:
        raise RuntimeError(
            "Cannot flatten mask with a threshold larger than the z dimension"
        )
...
def lorentzian_kern(gamma, length, demean=True):
    ...
    if demean:
        out = out - np.mean(out)  # zero mean/DC
    return out


def lorentzian_autocorr(length, gamma):
    autocorr = np.real(
        ifft(np.square(np.absolute(fft(lorentzian_kern(gamma, length, 1)))))
    )
...
def gaussian_fft(sigma, length, demean=True):
    ...
    x = [
        sigma * exp(-(0.5 * sigma**2 * (2 * pi * k) ** 2))
        + sqrt(2 * pi)
        * sigma
        * exp(-(0.5 * sigma**2 * (2 * pi * ((maxk + fres) - k)) ** 2))
        for k in krange
    ]
    if demean:
        x[0] = 0
...
def create_deblur_kern(wsp, mean_fft_z, kernel_length):
    ...
    if wsp.deblur_kernel == "direct":
        ...
        mean_fft_z = mean_fft_z / (
            mean_fft_z[1] + slope
        )  # normalise, we want DC=1, but we will have to extrapolate as we dont have DC
        ...
        wsp.log.write(
            " - Lorentizian-Weiner kernel: Gamma=%.4f, tunef=%.4f\n"
        ...
        kernel_fft = gaussian_fft(
            sigfit, kernel_length, True
        )  # When getting final spec. den. include mean
        wsp.kernel = ifft(kernel_fft)
    elif wsp.deblur_kernel == "manual":
        if len(sig) != kernel_length:
            raise RuntimeError(
                "Manual deblur kernel requires signal of length %i" % kernel_length
            )
...
def deblur_lr(wsp, volume):
    ...
    for i in range(wsp.ifnone("lr_iterations", 100)):
        ...
        if lam > 0:
            ...
            mod_grad = np.sqrt(
                np.square(grad[0]) + np.square(grad[1]) + np.square(grad[2])
            )
            mod_grad[mod_grad == 0] = 1
            grad = grad / mod_grad
            reg = np.ufunc.reduce(
                np.add, [np.gradient(grad[i], axis=i) for i in range(3)]
            )
            deblurred += lam * reg
...
def data_pad(img, padding_slices=2):
    ...
    return np.pad(
        data, [(0, 0), (0, 0), (padding_slices, padding_slices), (0, 0)], "edge"
    )
...
def get_kernel(wsp):
    ...
    if len(wsp.kernel) < kernel_length:
        ...
        wsp.kernel = np.concatenate(
            (wsp.kernel[:half_len], np.zeros(n), wsp.kernel[half_len:])
        )
...
class Options(OptionCategory):
    ...
    def groups(self, parser):
        ...
        group.add_option(
            ...
            choices=["direct", "gauss", "manual", "lorentz", "lorwien"],
            default="direct",
        )
        group.add_option(
            "--kernel-file",
            dest="kernel",
            help="File containing pre-specified deblurring kernel data",
            type="matrix",
        )
        group.add_option(
            "--deblur-gamma",
            help="Value of gamma to use in lorentzian kernels",
            type="float",
        )
        group.add_option(
            "--method",
        ...
        group.add_option(
            "--addimg",
            type="image",
            help="Additional image to deblur using same residuals. Output will be saved as <filename>_deblur",
        )
        group.add_option(
            "--save-kernel",
            help="Save deblurring kernel",
            action="store_true",
            default=False,
        )
        group.add_option(
            "--save-residuals",
            help="Save residuals used to generate kernel",
            action="store_true",
            default=False,
        )
        return [group]
...
def main():
    ...
    try:
        parser = AslOptionParser(
            usage="oxasl_deblur -i <ASL input file> [options...]", version=__version__
        )
